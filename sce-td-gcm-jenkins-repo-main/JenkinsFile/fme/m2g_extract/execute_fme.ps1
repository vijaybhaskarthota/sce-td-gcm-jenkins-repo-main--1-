...
Write-Host "FME_URL:$FME_URL,FME_TOKEN:$FME_TOKEN,ENVIRONMENT:$ENVIRONMENT,REPOSITORY:$FME_REPOSITORY,WORKSPACE:$FME_WORKSPACE"
$headers = @{
...
try {
  $Result=Invoke-WebRequest -UseBasicParsing -Uri "$FME_URL/fmerest/v3/transformations/transact/$FME_REPOSITORY/$FME_WORKSPACE" -Method Post -Headers $headers -Body $body
