...
WHERE INGESTION_COMPLETE_STATUS ='Y' AND JOBNAME = 'DER FILE INGESTION' AND 
jobid in (SELECT max(jobid) FROM  TCGACDS.GCM_DER_PROCESS_STATUS 
WHERE JOBNAME = 'DER FILE INGESTION' AND INGESTION_COMPLETE_STATUS ='Y'))
union
...
WHERE JOBNAME = 'DER FILE INGESTION' AND 
jobid in (SELECT MAX(jobid) FROM  TCGACDS.GCM_DER_PROCESS_STATUS 
WHERE JOBNAME = 'DER FILE INGESTION'))
union
...
SELECT MAX(JOBID) FROM TCGACDS.GCM_DER_PROCESS_STATUS WHERE JOBNAME = 'DER FILE INGESTION' 
...
AND INGESTION_COMPLETE_STATUS ='Y')
union
SELECT 'Total number of records loaded into ECM001_DER_INTERCONN Table' AS Validations, 
DER_DATA_COUNT AS COUNT,
(CASE WHEN PROCESS_COUNT = DER_DATA_COUNT THEN 'PASS' ELSE 'FAIL' END) AS REMARKS FROM
(
SELECT
FEATURE_COUNT as PROCESS_COUNT,
(select count(*)  from  TCGACDS.ECM001_DER_INTERCONN) as DER_DATA_COUNT
from TCGACDS.GCM_DER_PROCESS_STATUS
where JOBID IN (
SELECT MAX(JOBID) FROM TCGACDS.GCM_DER_PROCESS_STATUS WHERE JOBNAME = 'DER FILE INGESTION' 
AND INGESTION_COMPLETE_STATUS ='Y'))
union
SELECT 'Tech type not present in techtype lookup  or tech type value is null in input DER file' AS Validations, 
COUNT(*) AS COUNT, 'WARNING' AS Remarks FROM 
TCGACDS.ECM001_DER_INTERCONN WHERE TECHNOLOGY_TYPE NOT IN (SELECT GCM_TECHNOLOGY_TYPE FROM TCGACDS.GCM_DER_TEchnology_types_look_up)
