...
def deploy() {
    ...
    // }
    powershell label: 'Executing install/Rollback Products/APIs', script: "${getFilePath("deployProductstemp.ps1")}"
}
def enable_gcm_outage() {
    powershell label: 'Enable GCM Outage', script: "${getFilePath("deployProductstemp.ps1")}"
}
def remove_gcm_outage() {
    powershell label: 'Remove GCM Outage', script: "${getFilePath("deployProductstemp.ps1")}"
}